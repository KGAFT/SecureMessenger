...
public class FilesEncryptedNativeCalls {
    ...
    public File downloadAndDecryptFile(long appId, long fileId, byte[] key, String destinationFolder, String tempFolder) {
        ...
        try{
            ...
            fos.flush();
            File resultFile = new File(destinationFolder+"/"+generateResultFileName(getFileName(appId, fileId), fileId));
            IOUtil.decryptFile(key, tempFile.getAbsolutePath(), resultFile.getAbsolutePath());
            tempFile.delete();
            return resultFile;
        }catch (Exception e){
            tempFile.delete();
    ...
    private String generateResultFileName(String baseFileName,  long fileId){
        String fileExtension = IOUtil.getFileExtension(baseFileName);
